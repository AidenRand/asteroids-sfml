...
void Asteroids::collision(bool& asteroid_dead, bool& bullet_dead, std::vector<Bullet> bullet_vector, int& level)
{
	for (unsigned int i = 0; i < bullet_vector.size(); i++)
	{
		...
		if (bullet.getGlobalBounds().intersects(asteroid.getGlobalBounds()))
		{
			...
			bullet_dead = true;
			level = 2;
			spawn_point.x = asteroid.getPosition().x;
			spawn_point.y = asteroid.getPosition().y;
			asteroid.setPosition(spawn_point);
		}
	}
}

void Asteroids::cutAsteroid(int& level, float& asteroid_scale)
{
	if (level == 2)
	{
